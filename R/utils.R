...
init_check_r <- function(r, init_r, unique_names, player) {
  if (length(r) == 0) {
    ...
    warning(
      sprintf(
        "Some names in %s doesn't have a match in r.
...
init_check_rd <- function(rd, init_rd, unique_names, player) {
  if (length(rd) == 0) {
    ...
    warning(
      sprintf(
        "Some names in %s doesn't have a match in rd.
...
init_check_sigma <- function(sigma, init_sigma, unique_names, player, method) {
  if (method != "glicko2") {
    ...
    warning(
      sprintf(
        "Some names in %s doesn't have a match in sigma.
...
initialize_vec <- function(var, data, argname, min = -Inf, max = Inf) {
  if (length(var) == 0) {
    ...
    if (!var %in% colnames(data)) {
      stop(sprintf(
        "variable %s specified in %s is not present in data",
        var, argname
      ))
    }
