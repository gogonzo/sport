...
glicko_run <- function(data, r, rd){
  ...
  for(i in 1:length(data)){
    ...
    team_names<- data[[ i ]][["team_name"]]
    ranks  <- data[[ i ]][["rank"]]
    time   <- data[[ i ]][["time"]]
    
    glicko <- glicko( team_names , rank = ranks, r = r[team_names, ], rd = rd[team_names,], time = time )    
...
glicko2_run <- function(data, r, rd, sig){
  if(missing(r)){
    team_names <- unique(data$team_name)
    r <- setNames( rep(1500, length(team_names)), team_names )
    rd<- setNames( rep(300,  length(team_names)), team_names )
  ...
  for(i in 1:length(data)){
    ...
    r [ team_name, ] <- glicko$r[  team_name ]
    rd[ team_name, ] <- glicko$rd[ team_name ]
    sig[team_name, ] <- glicko$sig[ team_name ]
