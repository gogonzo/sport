...
To compute ratings using each algorithms one has to specify formula. To estimate riders (`rider_name`) abilities with given outputs (`rank`) nested within particular heats (`id`). 
Glicko uses only one parameter per competitor to describe his overall abilities (`rider_name`). Ouput is a ranking within specified event (`rank|id`). One can also specify initial parameters based on prior knowledge. `glicko` estimates `r` and `rd` but glicko2 has additional `sig` parameter, measuring volitality. If not specified, by default `r=1500`, `rd=300`, `sig=0.05`.
