...
# About
Package contains functions calculating ratings for two-player or multi-player matchups. Methods included in package are able to estimate players form and their evolution in time, also able to predict output of challange. 
...
list_glicko  <- glicko_run(  formula = rank|id ~ rider , data = head(gpheats,10002 ) )
list_glicko2 <- glicko2_run( formula = rank|id ~ rider , data = head(gpheats,10002) )
list_bbt     <- bbt_run( formula = rank|id~rider,  data = head(gpheats,10002 ) )
list_dbl     <- dbl_run(  formula = rank|id~rider,  data = head(gpheats,10002 ) )
```
