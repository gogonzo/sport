...
$\pi_i = \frac{R_i}{RD_{ij}}$. However, expected outcome functions slightly 
differs in details, which can be noticed in formulas.
...
$$R_i = \sum_{j=1}^{k_i}R_{ij} * s_{ij}$$

$$RD_i^2 = \sum_{j=1}^{k_i}RD_{ij}^2 * s_{ij}$$

In above formula $s_{ij}$ denotes knows measurable share in team total efforts - 
for example share of time spend on the field by player $ij$ in total time.

Depending on algorithm type, update rule is computed as usual on the team level,
and then reward is distributed proportionally by to the players ratings.

$$R_{ij}^{'} \leftarrow R_{ij} * \Omega_i * \frac{RD_{ij}^2}{RD_i^2}$$
$$RD_{ij}^{'} \leftarrow RD_{ij} * (1 - \Delta *\frac{RD_{ij}^2}{RD_i^2})$$

where:
$\kappa$ - constant controlling RD update size, to not be greater than some 
fraction of RD, preventing against quick parameter value reduction.
...
$${R'}_i = R_i + \sum_i{\frac{RD_i^2}{c_{ij}}*(Y_{ij} - \hat{Y_{ij}})}$$

$${RD'}_i = RD_i * 
[1 - \frac{RD_{ij}^2}{RD_i^2}\sum_j{ \gamma_j * 
(\frac{RD_i}{c_{ij}})^2 * 
\hat{Y_{ij}}\hat{Y_{ij}} } ]$$
...
to add other important factors to the prediction. Algorithm perform better in 
disciples where other variables can make a difference in result eg. home field 
advantage. 
DBL implements Extended Kalman Filter learning rule, and allows to estimate 
multiple parameters in addition to player ratings. DBL is a dynamic 
logit extended to usage in pairwise comparisons by modeling differences of 
parameters instead of parameters itself. For sake of consistency with other 
algorithms $R$ and $RD$ is used instead of $\beta$ and $\sigma$ (in EKF $\omega$ 
and $\Sigma$). In DBL $R$ denotes weights/parameters not only player ratings and
$RD$ is parameter standard deviation.

$$\hat{Y_{ij}} = \frac{e^{-K(s_t) (\pi_{it} - \pi_{it})}}
...
$$\hat{R}_{i t+1} = \hat{R}_{it} + \frac{RD^2_{it}}
{1 + \hat{Y_{ij}} (1 - \hat{Y_{ij}})} * 
x_t (Y_{ij} - \hat{Y_{ij}})$$


$$RD^2_{i t+1} = RD^2_{i(t-1)} - 
\frac{\hat{Y_{ij}}(1 - \hat{Y_{ij}})}
{1+\hat{Y_{ij}} (1-\hat{Y_{ij}})s_t^2} * 
(RD^2_{pt}x_i)(RD^2_{it}x_i)^T$$
...
expected matchup output and true result, but sometimes one can have prior 
knowledge about particular player or event to have a higher volatility. `lambda`
proportionally changes prior `RD` before update, to increase or decrease 
uncertainty of challenge. 
`lambda` can be used differently in several type of events:
...
Depending on the frequency of events, $RD$ tends to decrease quickly to almost
