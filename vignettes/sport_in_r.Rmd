...
event and allows ties also. 
For all games, *output needs to be a rank/position in event*. Don't mix up rank 
output with typical 1-win, 0-lost. In `sport` package output for two player game
is 1=winner 2=looser. Below example of two matches with 4 players each.
...
- RHS of the formula have to be specified with `team(player)` term or 
`team(player | team)` when players competes in team match. `team(...)` is a 
...
results (ranks) are stored and `id` (optional). RHS should be specified 
by `rank | id` to split matches, otherwise all data will be computed under
same event id.
...
estimations. For `glicko2` there is also `rating$final_sigma`.
* `r` contains `data.frame` with prior ratings estimations from first event 
...
In all methods formula must contain `rank | id ~ team(player)` elements, to 
...
- `team(...)` function helps to identify column in which names of the players 
are stored. `team(...)` can be specified in two ways:  
  
  1. `team(player)` if results of the event are observer per player.
  ...
  2. `team(player | team)` when players competes within teams, and results are 
...
To specify weight one needs to create additional column in input data, and pass 
the name of the column to `weight` argument. For example weight could depend on
importance of competition. In speedway Grand-Prix last three heats determine 
event winner, thus they weight more.
...
quickly decrease to zero, making further changes limited. Setting `kappa` (single
value) avoids rating deviation decrease to be lower than specified fraction of
`rd`. In  other words final `rd` can't be lower than initial `RD` times 
`kappa` $\small RD' \geq RD * kappa$. 

```{r}
bbt1 <- bbt_run(formula = rank | id ~ team(rider),
                data = data, 
                kappa = 0.99) # RD decreases at most 1%

bbt2 <- bbt_run(formula = rank | id ~ team(rider), 
                data = data, 
...
In some cases player ratings tends to be more uncertain. If scientist have prior
knowledge about higher risk of event or specific player performance, then one
might create another column with relevant values and pass the column name to 
`lambda` argument.
...
data <- data %>%
 group_by(rider) %>%
 mutate(idle_30d = as.integer(date - lag(date)) > 30,
 days_scaled = if_else(idle_30d, 1.0, 2.0)) %>%
...
are obtained per team. `sport` is able to compute player ratings, and requires 
only changing formula from `team(player)` to `team(player | team)`. `data.frame`
should always be a long format, with one player for each row. Ratings are update
according to their contribution in team efforts. `share` argument can be added
optionaly if scientist have some knowledge about players contribution in match 
