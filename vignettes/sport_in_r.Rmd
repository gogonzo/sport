...
```{r message=FALSE}
glicko <- glicko_run(formula = rank | id ~ rider, data = data)
glicko2 <- glicko2_run(formula = rank | id ~ rider, data = data)
bbt <- bbt_run(formula = rank | id ~ rider, data = data)
dbl <- dbl_run(formula = rank | id ~ rider, data = data)
print(dbl)
```
...
```{r message=FALSE, warning=FALSE}
library(dplyr) # for examples purpose 
...
glicko_run(formula = rank | id ~ name, data = data2, r = r, rd = rd)$final_r
```
...
```{r}
data <- mutate(data, weight = ifelse(heat >= (max(heat) - 3), 2, 1))
glicko <- glicko_run(formula = rank | id ~ rider, 
                     data = data, 
                     weight = "weight")
```
...
```{r}
bbt1 <- bbt_run(formula = rank | id ~ rider, data = data, kappa = 0.9)
bbt2 <- bbt_run(formula = rank | id ~ rider, data = data, kappa = 0.5)
...
```{r}
data %<>% mutate(beta = ifelse(heat >= (max(heat) - 3), 2, 1))
dbl <- dbl_run(rank | id ~ rider, beta = "beta", data=data) 
```
...
```{r}
sigma <- unique(data$rider) %>% setNames(runif(0.1, 0.5, n = length(.)), .)
glicko2 <- glicko2_run(rank | id ~ rider, data, sigma = "sigma")
```
...
bbt <- bbt_run(rank | id ~ rider, data=data, sigma = "days_scaled")
```